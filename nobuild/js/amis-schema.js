...
class AmisSchema extends HTMLElement {
  static get observedAttributes() { return ['api', 'schema']; }
  ...
  disconnectedCallback() { this.destroyAmis(); }
  ...
  async render() {
    ...
    if (api) {
      ...
      try { schema = JSON.parse(schemaAttr); } catch {
  ...
  async embedAmis(schema) {
    ...
    if (!amisRequire) { this.showAmisError(); return; }
    ...
    try {
      ...
      this.amisInstance = amis.embed(this.container, schema, {}, {
        ...
        fetcher: (config) => mockFetch(config.url, {
  ...
  showAmisError() {
    this.container.innerHTML =
      ...
      + '<p style="margin:0 0 4px;">缺少 AMIS 依赖，请下载 jssdk 并解压至 amis/ 目录：</p>'
  ...
  waitForAmisSDK() {
    return new Promise((resolve, reject) => {
      if (window.amisRequire) return resolve();
      let retries = 0;
      const check = () => {
        retries++;
        if (window.amisRequire) return resolve();
        if (retries >= 40) return reject(new Error('timeout'));
  ...
  destroyAmis() {
    if (this.amisInstance) { try { this.amisInstance.unmount(); } catch {} this.amisInstance = null; }
  }
}
