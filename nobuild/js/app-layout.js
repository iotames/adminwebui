...
class AppLayout extends HTMLElement {
  ...
  disconnectedCallback() {
    ...
    if (btn && this._logoutHandler) {
      btn.removeEventListener('click', this._logoutHandler);
    }
  }
    ...
    this.innerHTML += `
      <div class="al-shell">
        <aside class="al-sidebar">
          <div class="al-logo"><span class="al-logo-icon">◆</span><span>Admerp 中台</span></div>
  ...
  updateUserInfo() {
    ...
    if (user) this.updateUserDisplay(user.nickname);
  }

  setupListeners() {
    const btn = this.querySelector('#logoutBtn');
    if (btn) {
      this._logoutHandler = () => { authStore.getState().logout(); location.reload(); };
