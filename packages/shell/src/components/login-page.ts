import { post, authStore } from '@admerp/shared';
...
export class LoginPage extends HTMLElement {
  ...
  render() {
    this.shadowRoot!.innerHTML = `
      <style>
        ...
        .subtitle {
          text-align: center; font-size: 14px; color: #999; margin: 0 0 32px;
        }
        ...
        .error {
          color: #f5222d; font-size: 13px; margin-bottom: 16px;
    ...
    this.formEl = this.shadowRoot!.getElementById('loginForm') as HTMLFormElement;
    this.usernameEl = this.shadowRoot!.getElementById('username') as HTMLInputElement;
    this.passwordEl = this.shadowRoot!.getElementById('password') as HTMLInputElement;
    this.errorEl = this.shadowRoot!.getElementById('errorMsg') as HTMLElement;
  ...
  async handleLogin() {
    ...
    try {
      ...
      authStore.getState().setAuth(res.token, res.user);
