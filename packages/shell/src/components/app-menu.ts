import { get, menuStore } from '@admerp/shared';
...
export class AppMenu extends HTMLElement {
  ...
  async loadMenus() {
    try {
      this.menus = await get<typeof this.menus>('/api/system/menu');
  ...
  highlightActive(path?: string) {
    const current = path || location.pathname;
    const shadow = this.shadowRoot!;
    shadow.querySelectorAll('.menu-link').forEach(el => {
      const isActive = (el as HTMLElement).dataset.path === current;
  ...
  renderMenuItem(m: any, currentPath: string): string {
    ...
    return `
      <li class="menu-item">
        ...
        ${hasChildren ? `
          <ul class="sub-menu${childActive ? ' open' : ''}">
            ${m.children.map((c: any) => `
              <li>
                <button class="menu-link${c.path === currentPath ? ' active' : ''}" data-path="${c.path}" data-id="${c.id}">
                  <span>${c.title}</span>
                </button>
              </li>
            `).join('')}
